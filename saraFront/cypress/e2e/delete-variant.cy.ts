describe('Login to the app', () => {
 ...
 cy.get('input[placeholder="Color"]').last().clear().type('blue', { timeout: 10000 });
 cy.get('input[placeholder="Size"]').last().clear().type('36', { timeout: 10000 });
 cy.get('input[placeholder="Stock"]').last().clear().type('6', { timeout: 10000 });
 cy.get('input[placeholder="Price"]').last().clear().type('67', { timeout: 10000 });
 ...
 // Klik op de verwijderknop voor de eerste variant die een ID heeft
 cy.get('div.row').filter(':has(button.btn-danger)').first().within(() => {
  cy.get('button.btn-danger').click();
});
});

// Bevestigen dat de variant verwijderd is
cy.on('window:alert', (text) => {
expect(text).to.contains('Variant has been deleted');
});
