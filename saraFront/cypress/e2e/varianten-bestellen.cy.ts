describe('Login to the app', () => {
  it('should log in and store the JWT token', () => {
    ...
    cy.visit('http://localhost:4200/cart', { timeout: 10000 });
    // cy.get('div').contains('Rolex Cosmograph Daytona').should('be.visible'); // Controleer de productnaam
    // cy.get('div').contains('Color: pink').should('be.visible'); // Controleer de kleur
    // cy.get('div').contains('Size: 34').should('be.visible'); // Controleer de maat
    // cy.get('[data-cy=product-base-price]').should('contain.text', '$56.0');
    // cy.get('[data-cy=product-image]').should('have.attr', 'src', 'https://assets.wordans.nl/files/model_specifications/2015/10/7/155377/155377_big.jpg?1510922406');

    //Klik op de bestelknop
    cy.get('button').contains('Bestellen').click();

    // Wacht tot de bestelpagina is geladen en vul het formulier in
    cy.get('input[formControlName="Voornaam"]').type('Sara');
    cy.get('input[formControlName="Tussenvoegsel"]').type('de');
    cy.get('input[formControlName="Achternaam"]').type('Denno');
    cy.get('input[formControlName="Postcode"]').type('3341MT');
    cy.get('input[formControlName="Huisnummer"]').type('24');
    cy.get('textarea[formControlName="Opmerkingen"]').type('Geen opmerkingen');

    cy.get('button').contains('Bestelling betalen').click();
